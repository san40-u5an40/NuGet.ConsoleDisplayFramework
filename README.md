...
После вывода объектом `Printer` верхнего дисплея и контроллера, значение контроллера можно изменять с помощью стрелок ↑ и ↓. Для выхода из меню необходимо нажать [enter].
...
Третьим необязательным параметром компоновщик принимает логическое значение, указывающее на то, необходимо ли обновлять дисплей при изменении значения контроллера. Если используется `ControllerReadLine` со скрытым вводом, рекомендуется отключить обновление компоновщика.
